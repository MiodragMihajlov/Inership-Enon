  ...
  <div id="header">
    <div id="logo">
      ...
      <div class="user" id="user">
        ...
        <div v-if="this.user != null">{{this.user.usr_name}}</div>

      </div>
      
      <div class="menu-button-wraper">
        <button class="menu-button" @click="menu_options">&#9776;</button>
          <div class="menu-wraper" v-if="this.hamburgerToggle == true">
            <div class="menu-text-wraper">
              <ul>
                <li v-if="isAdmin == false"><router-link to="/">Home</router-link></li>
                
                <li v-if="isAdmin == false"><router-link to="/myplan">My Plan</router-link></li>
                
                
                <li v-if="isAdmin == false">
                  <router-link to="/workout">Settings</router-link>
                </li>
...
export default {
  ...
  methods: {
    ...mapActions(["changeIsloggedin", "userStore", "excStore", "session","changeIsAdmin"]),
    async logout() {
      try {
        ...
        const res = await axios.post(
          "http://576e122.e2.mars-hosting.com/api/plan-vezbanja/workout-logout", par);
        
        console.log.res
        this.$toast.show(res.data.message,{position:"top-left",duration:1200})
...
/* .menu-text-wraper .dropdown-content li a {
  padding-top: 2px !important;
} */
.menu-button {
